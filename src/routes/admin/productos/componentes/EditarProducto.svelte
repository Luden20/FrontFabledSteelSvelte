<script>
	...
	function agregarCategoria() {
		...
		let id = select.value.trim(); // aseguramos que sea string
	...
	function renderizarTags() {
		...
		for (let i = 0; i < cats.length; i++) {
			...
			nuevo.innerHTML = nombre + ` 
				<button type="button" onclick="eliminarTagNuevo('${cats[i]}')" class="btn-close btn-sm ms-2" aria-label="Eliminar"></button>`;
	...
	function renderizarImgs() {
		...
		contenedor.innerHTML = ""; // ✅ Limpieza antes de renderizar
	...
	<form on:submit={handleSubmit} class="registro-form needs-validation">
		<div class="row">
			<div class="col-md-6 mb-3">
				...
				<input
					bind:value={producto.PRD_NOMBRE}
					class="form-control"
					id="PRD_NOMBRE"
					name="PRD_NOMBRE"
					type="text"
			...
			<div class="col-md-6 mb-3">
				...
				<input
					bind:value={producto.PRD_DESCRIPCION}
					class="form-control"
					id="PRD_DESCRIPCION"
			...
			<div class="col-md-6 mb-3">
				...
				<input
					bind:value={producto.PRD_PRECIO}
					class="form-control"
					id="PRD_PRECIO"
					name="PRD_PRECIO"
					type="number"
		...
		<div id="categorias_asignadas" class="mb-3">
			{#each producto.categoria as categoria}
				<span class="badge-fabled m-1">
					...
					<button
						type="button"
						on:click={() => eliminarTagExistente(categoria.CAT_ID_PK)}
		...
		<div id="imagenes_asignadas" class="mb-4">
			{#each producto.imagenes as imagen}
				<div class="d-inline-block position-relative m-2">
					...
					<button
						type="button"
						class="btn-close btn-sm position-absolute end-0 top-0"
						aria-label="Eliminar"
						on:click={() => eliminarImgExistente(imagen.IMG_ID_PK)}
					></button>
				</div>
			{/each}
		</div>
